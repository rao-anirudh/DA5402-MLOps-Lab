...
class Dense_Neural_Diy:
    ...
    def random_weights(self):
        self.weights = random_weights(self.input_size, self.hidden_layer1_size, self.hidden_layer2_size,
                                      self.output_size)

    # Train the model method
    def fit(self, X, Y, learning_rate, epochs, batch_size):
        self.weights = train(X, Y, self.input_size, self.hidden_layer1_size, self.hidden_layer2_size, self.output_size,
                             learning_rate, epochs, batch_size, self.weights)
